...
for (s in f1_min:f1_max) 
{
  for (t in f2_min:f2_max)
  {
    ...
    total_coAuthored_Affiliations <- 0 # added in version 3_new 
    ...
    current_file <- paste(current_folder,"mag_papers_filtered_",toString(f1),"_",toString(f2),".txt",sep="")
    ...
    full_df <- data.frame(p_year, cbind(p_n_o_Authors), cbind(p_n_o_Affiliations),cbind(p_authors_affil), cbind(flag_all_NA), cbind(p_title), stringsAsFactors=FALSE)
    ...
    for (l in 1: nrow(coauthored_df))
    {
      ...
      title <- coauthored_df[6]$p_title[l]
      country_list <- coauthored_df[4]$p_authors_affil[[l]]
      ...
      if ((!is.na(country_list))&(!is.null(country_list)))
      {
        notna_country_list <- country_list[!is.na(country_list)]            
        notna_country_list <- notna_country_list[!notna_country_list %in% "NA"]
        if (length(unique(notna_country_list))>1)
        {
          Int_value <- 1
        }
      }
        ...
        else
        {
          for (m in 1:(n_o_Countries-1))
          {
            ...
            for (n in (m+1):n_o_Countries){
              ...
              International[k]<- Int_value
              Weight[k] <- 1/(factorial(n_o_Authors)/(factorial(2)*factorial(n_o_Authors-2)))
              
              Title_list[k] <- title
              
              if ((Country1[k]!='NA')&(Country2[k]!='NA')&(International[k]==1))
              {
                write.table(data.frame(Year [k], Country1[k], Country2[k], International[k], Weight[k], stringsAsFactors = F), file = bilateral_file, sep=",",append = T,row.names = F, col.names = F)                
              }
              k <-k+1
            }
